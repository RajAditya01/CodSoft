    ...
    useEffect(() => {
        if (isOpen) {
            axios.get(`http://localhost:9000/project/${id.projectId}/task/${id.id}`)
                .then((response) => {
                    const task = response.data[0]?.task[0];
                    if (task) {
                        console.log("Fetched Task Data:", task); // Debug: Log task data
                        setTaskData(task);
                    } else {
                            ...
                            <Dialog.Panel className="rounded-md bg-white max-w-[85%] w-[85%] h-[85%] overflow-y-hidden">
                                ...
                                <div className='flex gap-4 h-[inherit]'>
                                    <div className="!w-8/12 px-8 space-y-3 py-4 min-h-max overflow-y-auto">
                                        ...
                                        {taskData.deadline && (
                                            <p className='text-gray-500 mt-2'>Deadline: {new Date(taskData.deadline).toLocaleDateString()}</p>
                                        )}
                                    </div>
                                    <div className="w-4/12 py-4 pr-4">
                                        {/* Add any additional information or actions for the task here */}
                                    </div>
                                </div>
                            </Dialog.Panel>
                        </Transition.Child>
